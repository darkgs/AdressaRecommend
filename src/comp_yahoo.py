...
parser.add_option('-y', '--y2v_path', dest='y2v_path', type='string', default=None)
...
parser.add_option('-d', '--dropout_rate', dest='dropout_rate', type='float', default=0.3)


class UserRepModel(nn.Module):
    def __init__(self, embed_size, cate_dim, args):
        super(UserRepModel, self).__init__()
...
def main():
    ...
    if (options.input == None) or (options.d2v_embed == None) or \
            (options.u2v_path == None) or (options.y2v_path == None) or \
