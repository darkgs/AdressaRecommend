...
class NeRTModel(nn.Module):
    def __init__(self, embed_size, cate_dim, args):
        ...
        self._v_attn = nn.Parameter(torch.zeros([hidden_size, 1], dtype=torch.float32), requires_grad=True)
        self._W_attn = nn.Parameter(torch.zeros([hidden_size*2, hidden_size], dtype=torch.float32), requires_grad=True)
        ...
        nn.init.xavier_normal_(self._v_attn.data)
    ...
    def forward(self, x1, x2, cate, seq_lens, _, attn_mode=False):
        ...
        for step in range(sequence_lenths.shape[0]):
            ...
            for prev in range(prev_hs.size(1)):
                ...
                attn_inter = torch.tanh(torch.matmul(attn_input, self._W_attn))
                attn_inter = torch.matmul(attn_inter, self._v_attn) + self._b_attn
                attn_score.append(attn_inter)
