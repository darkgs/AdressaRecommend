...
class MultiCellLSTM(nn.Module):
	def __init__(self, embed_size, hidden_size, attn_count, x2_dropout_rate):
		...
		self._W_cell_attn = nn.Parameter(torch.zeros([hidden_size*2, 1]), requires_grad=True)
		...
		nn.init.xavier_normal_(self._W_cell_attn.data)
	...
	def forward_with_attn(self, x1, x2, states):
		...
		#
		o1_score = torch.matmul(torch.cat([h_t, c1_t], 1), self._W_cell_attn)
		o2_score = torch.matmul(torch.cat([h_t, c2_t], 1), self._W_cell_attn)

		softmax_sum = torch.exp(o1_score) + torch.exp(o2_score)

		h_t = (torch.exp(o1_score) * o1_t * torch.tanh(c1_t) + torch.exp(o2_score) * o2_t * torch.tanh(c2_t)) / softmax_sum
