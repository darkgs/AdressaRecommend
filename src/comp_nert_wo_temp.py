...
class NeRTModel(nn.Module):
    ...
    def forward(self, x1, _, __, seq_lens, ___, ____):
        batch_size = x1.size(0)
        max_seq_length = x1.size(1)
        embed_size = x1.size(2)

        outputs = torch.zeros([max_seq_length, batch_size, self._rnn_hidden_size],
        ...
        for step in range(sequence_lenths.shape[0]):
            ...
            for prev in range(prev_hs.size(1)):
                attn_input = torch.cat((prev_hs[:,prev,:], outputs[step-1][:sequence_lenth]), dim=1)
