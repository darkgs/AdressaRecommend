...
$(BASE_PATH)/data_per_day: $(ADRESSA_DATAS) src/raw_to_per_day.py
	$(info [Makefile] $@)
	$(call asked_delete, $@)
	@python3 src/raw_to_per_day.py -o $@ -m $(MODE) -d $(DATA_SET)

$(BASE_PATH)/data_for_all: $(ADRESSA_DATAS) data/article_content.json $(BASE_PATH)/data_per_day src/merge_days.py
	$(info [Makefile] $@)
	$(call asked_delete, $@)
